...
use BlackCat\Database\Packages\Sessions\Repository\SessionRepositoryInterface;
...
class SessionRepository implements SessionRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Sessions\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'token_hash_key_version', 'token_fingerprint', 'token_issued_at', 'user_id', 'last_seen_at', 'expires_at', 'failed_decrypt_count', 'last_failed_decrypt_at', 'revoked', 'ip_hash', 'ip_hash_key_version', 'user_agent', 'session_blob' ]);
          return count($rows);
        }
