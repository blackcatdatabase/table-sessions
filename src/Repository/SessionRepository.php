...
namespace BlackCat\Database\Packages\Sessions\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\Sessions\Definitions;
use BlackCat\Database\Packages\Sessions\Criteria;
use BlackCat\Database\Packages\Sessions\Dto\SessionDto as Dto;
use BlackCat\Database\Packages\Sessions\Mapper\SessionDtoMapper as RowMapper;
...
final class SessionRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\Sessions\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'token_hash' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\Sessions\Definitions::table());
            return $bulk->upsertMany($rows, [ 'token_hash' ], [ 'token_hash_key_version', 'token_fingerprint', 'token_issued_at', 'user_id', 'last_seen_at', 'expires_at', 'failed_decrypt_count', 'last_failed_decrypt_at', 'revoked', 'ip_hash', 'ip_hash_key_version', 'user_agent', 'session_blob' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Sessions\Dto\SessionDto|null */
    public function getByTokenHash(string $tokenHash, bool $asDto = false): array|\BlackCat\Database\Packages\Sessions\Dto\SessionDto|null {
        $row = $this->getByUnique([ 'token_hash' => $tokenHash ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\Sessions\Mapper\SessionDtoMapper::fromRow($row);
    }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\Sessions\Dto\SessionDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\Sessions\Dto\SessionDto|null {
